...
if (! function_exists('resize_uploaded_image')) {
    function resize_uploaded_image($request, string $inputName, UploadedFile $file, string $path, int $width = 200, $disk = 'public'): string
    {
        $path = rtrim($path, '/') . '/';
        ...
        Storage::disk($disk)->put($path.$name, $img);
